...
sudo pacman -S --needed base-devel git vim kitty polybar rofi noto-fonts-emoji xdotool picom nitrogen bash-completion pkgfile pcmanfm ranger xss-lock polkit lxqt-policykit dunst man-db tldr neovim plocate font-manager python nerd-fonts adobe-source-code-pro-fonts ttf-opensans noto-fonts gnu-free-fonts vlc libmicrodns protobuf mpv firefox libreoffice flameshot htop binutils rofimoji zathura zathura-pdf-mupdf zathura-cb zathura-djvu peek python-pywal python-pip
...
yay -S --needed ly i3lock-color rofi-greenclip papirus-icon-theme qview devtodo nerd-fonts-complete
